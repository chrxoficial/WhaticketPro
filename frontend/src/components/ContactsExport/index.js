...
const ContactsExport = (props) => {
  ...
  useEffect(() => {
    if (selecteds.length === 0 && queueSelected.length === 0) {
      ...
      console.log(
        "Abaixo estão o resultado de tickets da api, esses dados não contem nenhum tipo de tratamento:"
      )
      console.log("Tickets")
      console.log(tickets)
      console.log("tags")
      ...
      for (let contact of tempTickets) {
        const t = contact
        finalTempTickets += `${t.contact.name},${t.contact.number},${
          t.contact.email
        },${t.tags.map((tag) => tag.name)},${
          t.queue !== null ? t.queue.map((queue) => queue.name) : ""
        }\n`
      }
  ...
  const filterTickets = (ticket, filter, typeFilter) => {
    if (ticket[typeFilter].length) {
      if (
        ticket[typeFilter].filter(
          (opgParaFiltrar) =>
            filter.filter((tag) => tag.name === opgParaFiltrar.name).length >= 1
        ).length >= 1
      ) {
        return true
      } else {
  ...
  const loadContacts = async () => {
    try {
      ...
      let tempTickets =
        "Nome, Numero, E-mail, Tags, Setores\n" + finalTempTickets
